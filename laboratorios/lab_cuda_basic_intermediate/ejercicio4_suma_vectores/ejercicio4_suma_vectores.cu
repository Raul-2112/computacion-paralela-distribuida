...
int main() {
    ...
    // Calcular dimensiones y lanzar kernel
    ...
    // Verificar primeros y últimos elementos
    ...
    int errores = 0;
    for (int i = 0; i < N; i++) {
        if (h_C[i] != 3.0f) errores++;
    }
    if (errores == 0)
        printf("[OK] Todos los %d elementos son correctos.\n", N);
    else
        printf("[ERROR] %d elementos incorrectos.\n", errores);
